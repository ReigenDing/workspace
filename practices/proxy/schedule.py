...
def insert_into_db():
    ...
    if not database:
        return None
    proxy_saver = practices.db.SQLite(database=database)
    while True:
        clear_mark = False
        # 每天零点清空当天的存活代理
        midnight = 86400
...
def db_exit():
    ...
    if not os.path.exists(proxy_db_path):
        proxy_db = practices.db.SQLite(database=proxy_db_path)
    ...
    if not os.path.exists(alive_db_path):
        alive_db = practices.db.SQLite(database=alive_db_path)
